...
extension SCNetworkReachabilityFlags {
    ...
    var description: String {
        ...
        let reachFlag = isReachableFlagSet ? "R" : "-"
        let connectionFlag = isConnectionRequiredFlagSet ? "c" : "-"
        let transientFlag = isTransientConnectionFlagSet ? "t" : "-"
        let interventionFlag = isInterventionRequiredFlagSet ? "i" : "-"
        let onTrafficFlag = isConnectionOnTrafficFlagSet ? "C" : "-"
        let onDemandFlag = isConnectionOnDemandFlagSet ? "D" : "-"
        let localAddressFlag = isLocalAddressFlagSet ? "l" : "-"
        let directFlag = isDirectFlagSet ? "d" : "-"

        return "\(wanFlag)\(reachFlag) \(connectionFlag)\(transientFlag)\(interventionFlag)\(onTrafficFlag)\(onDemandFlag)\(localAddressFlag)\(directFlag)"
    }
}
